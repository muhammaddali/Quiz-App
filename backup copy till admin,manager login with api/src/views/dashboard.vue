...
const roleLinks = {
  ...
  supervisor: [
    { path: '/dashboard/supervisorViewQuizzes', icon: 'fas fa-eye', label: 'View Quizzes' },
    { path: '/dashboard/supervisorApproveRequests', icon: 'fas fa-thumbs-up', label: 'Approve Requests' },
    { path: '/dashboard/supervisorViewResults', icon: 'fas fa-chart-bar', label: 'View Results' },
  ],
};
      ...
      <div class="row">
        ...
        <div :class="['sidebar-container', { 'collapsed': isCollapsed }]" style="padding-left: 0">
          <aside class="sidebar bg-dark p-3" v-if="user && !loading">
            <h5 class="text-warning" v-if="!isCollapsed">Role: {{ user.role || 'Role not defined' }}</h5>
...
.main-content-container {
  margin-left: 250px; /* Margin for the sidebar */
  width: calc(100% - 250px); /* Main content width */
...
.collapsed.sidebar-container {
  width: 0; /* Sidebar width when collapsed */
}

.collapsed.main-content-container {
  margin-left: 0; /* Main content full width when collapsed */
...
.toggle-button {
  font-size: 24px;
  position: fixed;
  top: 20px; /* Adjusted to avoid overlap */
  left: 10px;
  background: none;
  border: none;
  cursor: pointer;
...
h4, h5 {
  margin-top: 20px; /* Adjusted to avoid overlap with toggle button */
}
...
@media (max-width: 768px) {
  ...
  .main-content-container {
    margin-left: 0; /* Main content full width on mobile */
    width: 100%; /* Full width for main content */
  }
  ...
  .collapsed.main-content-container {
    margin-left: 0; /* Adjust content margin when collapsed */
  }
}

@media (min-width: 769px) {
  .sidebar-container {
    display: block; /* Show the sidebar on tablet and larger screens */
    width: 250px; /* Ensure the sidebar is at its full width */
  }

  .collapsed.sidebar-container {
    display: block; /* Ensure the sidebar still occupies space when collapsed */
    width: 0; /* Prevent the sidebar from showing */
  }

  .main-content-container {
    margin-left: 250px; /* Ensure the main content margin is adjusted */
